...
const LinkMenu: React.FC<EditorType> = ({ editor }) => {
  ...
  const handleOpenMenu = useCallback(() => {
    ...
    if (previousUrl) {
      ...
      setLinkType(
        previousUrl.startsWith(`/${locale}/post/`) || previousUrl.startsWith("#")
          ? "internal"
          : "external"
      );
    }
  ...
  const handleSaveLink = useCallback(() => {
    ...
    if (linkType === "internal") {
      ...
      if (finalUrl.startsWith("#")) {
        ...
        // e.g., #section-name
      } else if (finalUrl.match(new RegExp(`^\/${locale}\/post\/\\d+\/[^\/]+$`))) {
        // Already a complete localized post URL - keep as is
        // e.g., /en/post/123/slug-name
      } else if (finalUrl.match(new RegExp(`^\/post\/\\d+\/[^\/]+$`))) {
         // Already has /post/ prefix, prepend locale
         // e.g., /post/123/slug-name -> /en/post/123/slug-name
         finalUrl = `/${locale}${finalUrl}`;
      } else if (finalUrl.match(/^\d+\/[^\/]+$/)) {
        // Has ID and slug but missing /post/ prefix
        // e.g., 123/slug-name
        finalUrl = `/${locale}/post/${finalUrl}`;
      } else if (finalUrl.match(/^\d+$/)) {
    ...
    setLinkValue("");
    setLinkText("");
  }, [editor, linkValue, linkType, linkText, locale]);
  ...
  const getDisplayValue = useCallback(() => {
    if (linkType === "internal") {
      // Remove /{locale}/post/ prefix and show just ID/slug or anchor
      const localePrefix = `/${locale}/post/`;
      if (linkValue.startsWith(localePrefix)) {
          return linkValue.substring(localePrefix.length);
      }
      // Also handle case where linkValue might start with just /post/
      const postPrefix = "/post/";
      if (linkValue.startsWith(postPrefix)) {
          return linkValue.substring(postPrefix.length);
      }
      return linkValue.replace(/^#/, ""); // Handle anchors
    } else {
      // Remove https:// prefix for external links
      return linkValue.replace(/^https?:\/\//, "");
    }
  }, [linkValue, linkType, locale]);
        ...
        <div className="p-4">
          ...
          <div className="mb-4">
            ...
            <div className="mt-2 text-xs text-gray-500">
              {linkType === "internal"
                ? `Links to other posts (e.g., /${locale}/post/123/article-slug) or anchors (e.g., #section)`
          ...
          <div className="mb-4">
            ...
            <div className="flex items-center">
              {linkType === "external" ? (
                ...
                // Show the locale and /post/ prefix in the UI
                <span className="bg-gray-100 px-3 py-2 text-sm text-gray-500 border border-r-0 border-gray-300 rounded-l">
                  /{locale}/post/
                </span>
              ) : (
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (linkType === "internal") {
                    if (value.startsWith("#")) {
                      setLinkValue(value); // Anchors remain unchanged
                    } else {
                      // Store the user's input directly, the save function handles full path
                      setLinkValue(value);
                    }
                  } else {
                    setLinkValue(value);
